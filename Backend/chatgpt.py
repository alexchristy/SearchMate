...
class GPT4:
    ...
    def get_greeting(self, user_url:str):
        '''
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                "role": "user",
                "content": f"Can you generate me a simple short greeting someone who is looking for something. Just give me a greeting no quotes of any kind. The User URL is the website the user is currently on. Use this to tailor the message. Max 2 concise and short sentences. Only return back a reponse directly for the user. No quotes.\n\n   User URL: {user_url}"
                }
            ],
            temperature=1.5,
            max_tokens=50,
    ...
    def get_welcome_back(self, user_url:str):
        '''
        This function queries the GPT-3.5-Turbo model for a greeting.
        '''
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                "role": "user",
                "content": f"Can you generate me a simple, short, and concise welcome back message. Just give me the welcome back message no quotes of any kind. The User URL is the website the user is currently on. Use this to tailor the message. Max 2 concise and short sentences. Only return back a reponse directly for the user. No quotes.\n\n   User URL: {user_url}"
                }
            ],
            temperature=1.5,
            max_tokens=50,
    ...
    def get_relevant_product_link(self, user_query: str, base_url: str, search_results: list):
        '''
        This function queries the GPT-4 model for a relevant result.
        '''
        ...
        message_to_send = [
                {
                "role": "user",
                "content": f"As the world's best intention predictor, I am seeking your expertise to analyze the provided data and select the most relevant product or service. The user's query refers to their specific search request on the website they are currently browsing. The User URL indicates the website that the user was on when they made the query, and it is essential to consider this context throughout the process. Your task is to examine the user's query, taking into account the website's context, and evaluate the search results obtained from Google to choose the result with the most relevant product or service. Pay close attention to the title and snippet, as they provide a glimpse into the content of each result. Additionally, verify that the link corresponds to the same site the user searched from. If there is no relevant result return None. Your final task is to provide only the link to the most relevant search result. Remember return only the link.\n\nUser Query: {user_query}\nUser URL: {base_url}"
                }
            ]
    ...
    def is_user_shopping(self, user_query: str, base_url: str):
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                "role": "user",
                "content": f"I am seeking your expertise in analyzing a user's search intent on a specific website. Your exceptional ability to discern intentions makes you the best person for this task. The user's search could encompass various aspects such as a product, a location, or specific information available on the website. To aid you in making an accurate determination, I will provide you with the following information: the User Query, which represents what the user is saying or desiring, and the User URL, which indicates the current site the user is on. Your objective for this task is to examine the user query and user URL and ascertain whether the user is searching for a purchasable product or service. If it is evident that the user is searching searching for a purchasable product or service, please return True. Conversely, if it seems that the user is not searching searching for a purchasable product or service, please return False. Please refrain from returning any other information beyond these outcomes.\n\nUser Query: {user_query}\nUser URL: {base_url}"
                }
            ],
            temperature=0,
            max_tokens=256,
    ...
    def is_page_empty(self, page_content: str):
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                "role": "user",
                "content": f"You are the best web developer and front end engineer in the world. I am seeking your expertise to determine if a link is broken, removed, or no longer has any useful content. The Page Content is the displayed text content a user sees when they visit that link. If it appears the link leads to a broken, removed, or no longer useful page return True. If the page appears to still be hosting content return False. Only return True or False based on your decision. Do not return anything else.\n\nPage Content: {page_content}"
                }
            ],
            temperature=0,
            max_tokens=256,
